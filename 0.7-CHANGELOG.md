...
### 🧠 Smart Brains & Sharp Eyes (2025-11-19)
...
- **No more hardcoded guessing games**: Model icons (🧠 for reasoning, 👁️ for vision) now come straight from the API's own metadata
- **Better provider support**: OpenAI o1/o3, DeepSeek reasoner, Perplexity sonar-reasoning — all now properly identified as brainy boys
...
### 💰 Pricing That Makes Sense (2025-11-19)
...
- **Beautiful, compact format**: Clean single-line pricing (`$2.00 → $12.00/1M`) that doesn't take up half the screen
...
- **Friendly model names**: Instead of `x-ai/grok-code-fast-1`, you get "Grok Code Fast 1 (xAI)" — much nicer to look at

**The glow-up**: Model selector went from sluggish to lightning-fast, and you can actually see what things cost now.

---

### 🔄 Automatic Updates (The Sparkle Effect) (2025-11-18)
...
### 🌐 Web Search Just Got Pretty (2025-11-19)
...
- **Clickable citations**: Click a badge to open that source in your browser — no copy-paste gymnastics required
...
- **Debounced search**: Type smoothly — the search waits 100ms to see if you're done before filtering results
...
## 🗑️ What We Got Rid Of (The Good Cleanup)

### Spotlight Integration (2025-11-18)

We deleted the entire Spotlight indexing feature. Here's the corpse:

- 🔍 `SpotlightIndexManager` — removed entirely
- 📚 CoreSpotlight framework — one less dependency cluttering the codebase
- 🧹 All Spotlight references stripped from:
...
- 🗄️ Spotlight attributes purged from the Core Data model
...
- 🗑️ **The tracker is dead**: No more following which models you used and when
- 📉 **Code graveyard**: Deleted `RecentModelsManager` and all its little tracking helpers
- 🎯 **Cleaner focus**: Model selector now only cares about your Favorites and the full list — much simpler

**Why?** Made the code simpler and the UI cleaner. Sometimes less is more.

---

## 🎯 The Great Code Diet (2025-11-19)
...
- **One UUID extraction method**: All UUID logic is now in `MessageContent.swift` — use it and only it
...
### Backend Refactoring (2025-11-20)

We consolidated and streamlined the backend architecture to eliminate code duplication and improve maintainability:

#### Service Layer Improvements
- **ChatService**: Created unified service for message sending (streaming and non-streaming)
- **SSEStreamParser**: Centralized Server-Sent Events parsing logic from all API handlers
- **BaseAPIHandler**: New base class for all API handlers with common functionality
- **Refactored all handlers**: ChatGPT, Claude, Deepseek, Mistral, Perplexity, Ollama, and OpenRouter now inherit from BaseAPIHandler
- **TavilySearchService**: Search orchestration moved from MessageManager to dedicated service

#### Data Layer Cleanup
- **Removed deprecated methods**: Cleaned up SelectedModelsManager (removed UserDefaults logic, redundant methods)
- **Streamlined ModelMetadata**: Removed all unused hardcoded PricingInfo static properties
- **Consolidated fetchers**: Replaced redundant metadata fetchers with GenericMetadataFetcher

#### Code Quality
- **Deleted dead code**: Removed unused FavoriteModelsManagerTest.swift
- **Project file maintenance**: Added new files to Xcode project (ChatService, SSEStreamParser, BaseAPIHandler)
- **Fixed compilation errors**: Resolved override keywords, async/sync mismatches, and protocol conformance issues

**Impact**: Reduced code duplication, improved maintainability, and standardized API handler architecture across all providers.

---

## 🐛 Fixes (The Bug Fixes That Actually Matter)
...
✅ Build verified and working (CoreSpotlight isn't even a memory anymore)  
✅ Zero Spotlight references lingering anywhere  
...
Check out `0.7-ROADMAP.md` for what's cooking. There's some spicy stuff coming. 🌶️
